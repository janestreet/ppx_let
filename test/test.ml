...
let () =
  (* Demonstrate that [%map(n)l] and [%bind(n)l] work correctly. If
     we were to use [%map(n)] and [%bind(n)] below instead, the [@zero_alloc]
     checks would fail (at least under [fast-build] at the time of writing)
     because the closure generated by the ppx would be heap allocated in
     the absence of the [@nontail] attribute. *)
