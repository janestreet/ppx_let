...
module Applicative_example = struct
  ...
  (* {[
       let _af a : _ X.t =
         let%bind x = a in
         (* "Error: Unbound value Let_syntax.bind" *)
         return (x + 1)
       ;;
     ]} *)
...
module X_local = struct
  type 'a t = 'a

  module Let_syntax = struct
    let[@zero_alloc] return (x : 'a) : 'a t = x

    module Let_syntax = struct
      let[@zero_alloc assume] bind x ~(local_ f) = f x
      let[@zero_alloc assume] map x ~(local_ f) = f x
      let[@zero_alloc assume] bind2 x y ~(local_ f) = f x y
      let[@zero_alloc assume] map2 x y ~(local_ f) = f x y
    end
  end
end

type f = int -> int -> int -> int -> int -> int -> int -> int

let () =
  (* Demonstrate that [%map(n)l] and [%bind(n)l] work correctly. If we were to use
     [%map(n)] and [%bind(n)] below instead, the [@zero_alloc] checks would fail (at least
     under [fast-build] at the time of writing) because the closure generated by the ppx
     would be heap allocated in the absence of the [@nontail] attribute. *)
  let open X_local.Let_syntax in
  let[@zero_alloc] f1 a b c d e f g =
    let value = return 1 in
    let%mapl_fun value in
    value + a + b + c + d + e + f + g
  in
  let[@zero_alloc] f2 a b c d e f g =
    let value = return 1 in
    let%bindl_fun value in
    value - a - b - c - d - e - f - g
  in
  let[@zero_alloc] f3 a b c d e f g =
    let value1 = return 1 in
    let value2 = return 2 in
    let%mapnl_fun value1 and value2 in
    value1 + value2 + a + b + c + d + e + f + g
  in
  let[@zero_alloc] f4 a b c d e f g =
    let value1 = return 1 in
    let value2 = return 2 in
    let%bindnl_fun value1 and value2 in
    value1 - value2 - a - b - c - d - e - f - g
  in
  ignore ([ f1; f2; f3; f4 ] : f list)
;;
