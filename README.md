...
Combines the affects of both `%mapl_fun/bindl_fun` and `%mapl_val/%bindl_val` together:
the closure generated by the ppx will be stack allocated _and_ the generated closure will
not have its own region, and thus the return value may be stack allocated.
